...
client.on('message', async message => {
  /*╔═══════════════════════════════════════╗
    Mentions
  ╚═══════════════════════════════════════╝*/
  if ((message.content.toLowerCase().includes("na_da") || message.content.toLowerCase().includes("nada")) && !message.author.bot && !message.content.toLowerCase().includes("canada")){
    client.users.get("338649491894829057").send("You were mentioned by " + message.author.username +" in " + message.channel.name + " : " + message.content + "\n" + message.url)
  }
  /*╔═══════════════════════════════════════╗
     Repost
  ╚═══════════════════════════════════════╝*/
  ...
  ╚═══════════════════════════════════════╝*/
    ...
    /*╔═══════════════════════════════════════╗
     Dialogflow
  ╚═══════════════════════════════════════╝*/
    if (message.channel.type == 'dm' && client.user.id != message.author.id && message.content.startsWith(prefix) == false) {
      ...
      const sessionPath = sessionClient.sessionPath(process.env.PROJECT_ID, toString(userRandom))
      const request = {
        session: sessionPath,
        queryInput: {
          text: {
      ...
      if (response[0].queryResult.intent == null){  //sometimes the intent is null, if I remember correctly that happens for the fallback answers but I didn't work on this in a while and I forgot :D
        ...
        message.reply(rep)
        client.users.get("338649491894829057").send(`**${message.author.username}:** ${message}`) //just for me for testing
  ...
  ╚═══════════════════════════════════════╝*/
    ...
    try {
      ...
      console.error(error);
      client.channels.get('593500776920252416').send('Command handler: ' + error) 
    }
  }
});
/*╔═══════════════════════════════════════╗
  ...
  if(!user.bot && reaction.message.id === "679385800412233736") {
    switch (reaction.emoji.id|| reaction.emoji.name) {
      case '551077554543394827':
        client.channels.get("593500776920252416").send(user.username + " reacted with Echo Arena")
        reaction.message.guild.member(user).addRole(roleList.get('ea')); 
        break;
      case '551077555487244289':
        client.channels.get("593500776920252416").send(user.username + " reacted with Onward")
        reaction.message.guild.member(user).addRole(roleList.get('ow')); 
        break;
      case '551077555218939904':
        client.channels.get("593500776920252416").send(user.username + " reacted with Echo Combat")
        reaction.message.guild.member(user).addRole(roleList.get('ec')); 
        break;
      case '551077662798643200':
        client.channels.get("593500776920252416").send(user.username  + " reacted with Space Junkies")
        reaction.message.guild.member(user).addRole(roleList.get('sj')); 
        break;
      case '679384856551227392':
        client.channels.get("593500776920252416").send(user.username + " reacted with PW")
        reaction.message.guild.member(user).addRole(roleList.get('pw')); 
        break;
      case '🇪🇺':
        client.channels.get("593500776920252416").send(user.username + " reacted with EU")
      ...
      case '519923682001289217':
        client.channels.get("593500776920252416").send(user.username + " reacted with NA")
...
client.on('messageReactionRemove', (reaction, user) => {
  if(!user.bot && reaction.message.id === "679385800412233736") {
    switch (reaction.emoji.id || reaction.emoji.name) {
      case '551077554543394827':
        client.channels.get("593500776920252416").send(user.username + " removed Echo Arena")
        reaction.message.guild.member(user).removeRole(roleList.get('ea')); 
        break;
      case '551077555487244289':
        client.channels.get("593500776920252416").send(user.username + " removed Onward")
        reaction.message.guild.member(user).removeRole(roleList.get('ow')); 
        break;
      case '551077555218939904':
        client.channels.get("593500776920252416").send(user.username + " removed Echo Combat")
        reaction.message.guild.member(user).removeRole(roleList.get('ec')); 
        break;
      case '551077662798643200':
        client.channels.get("593500776920252416").send(user.username + " removed Space Junkies")
        reaction.message.guild.member(user).removeRole(roleList.get('sj')); 
        break;
      case '679384856551227392':
        client.channels.get("593500776920252416").send(user.username + " reacted with Pistol Whip")
        reaction.message.guild.member(user).removeRole(roleList.get('pw')); 
        break;
      case '🇪🇺':
        client.channels.get("593500776920252416").send(user.username + " removed EU")
        reaction.message.guild.member(user).removeRole(roleList.get('eu')); 
        break;
      case '519923682001289217':
        client.channels.get("593500776920252416").send(user.username + " removed NA")
        reaction.message.guild.member(user).removeRole(roleList.get('na')); 
        break;
    }   
  }
...
client.on('guildMemberAdd', member => {
  client.channels.get('566294428453699584').send(`${member} joined\nMember count: ${member.guild.memberCount}`)
})
client.on("guildMemberRemove", (member) => {
  client.channels.get('566294428453699584').send(`${member.user.username} left\nMember count: ${member.guild.memberCount}`)
})
client.on("guildBanAdd", function(guild, user, member){
  client.channels.get('566294428453699584').send(`${member.user.username} was banned\nMember count: ${member.guild.memberCount}`)
})
/*╔═══════════════════════════════════════╗
    Deleted Messages
  ╚═══════════════════════════════════════╝*/
client.on('messageDelete', async (message) => {
  ...
  try {
    ...
    client.channels.get("593500776920252416").send("Deleted Messages send: " + error)   
  }
})
